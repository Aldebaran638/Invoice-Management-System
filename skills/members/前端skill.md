...
4. 前端AI允许新增 `frontend/src/tools/<tool-key>/` 目录，并在该目录下实现本次工具的前端代码。
...
1. 一个工具必须对应一个前端工具目录。
2. 工具目录必须创建在 `frontend/src/tools/<tool-key>/` 。
...
8. 一个工具必须对应一个前端测试目录。
9. 前端测试目录必须创建在 `frontend/tests/<tool-key>/` 。
10. 主测试文件必须为 `frontend/tests/<tool-key>/index.spec.ts` 。
11. 前端测试目录允许存在辅助测试文件。
...
6. 第六步，在 `frontend/src/tools/<tool-key>/` 下创建工具目录。
7. 第七步，实现 API 调用层。
8. 第八步，实现页面组件。
9. 第九步，实现页面状态。
10. 第十步，在 `frontend/tests/<tool-key>/` 下生成本工具对应的前端测试文件。
11. 第十一步，补充前端测试。
12. 第十二步，输出前端报告。
13. 第十二步完成前，前端AI禁止提交。
...
3. 前端AI必须严格按照设计文档中的 Path参数、Query参数、Body参数、Header参数 组织请求。
4. 新建工具存在请求体时，前端AI必须使用统一请求结构：
   {
     "request_id": "req-001",
     "ts": 1710000000000,
     "payload": {}
   }
5. 前端AI必须按设计文档中的成功返回字段读取数据。
6. 前端AI必须按设计文档中的失败返回字段处理报错。
7. 前端AI禁止读取设计文档中不存在的响应字段。
8. 前端AI禁止发送设计文档中不存在的请求字段。
9. 前端AI禁止在页面中写死与设计文档不一致的假数据。
...
3. 前端AI生成的测试文件必须放在 `frontend/tests/<tool-key>/` 。
...
6. 前端AI必须覆盖以下流程：
   ...
   核心删除流程
   至少一条失败流程
7. 前端AI必须验证本次工具入口能从侧边栏访问。
8. 前端AI必须验证本次工具页面能正常渲染。
9. 前端AI禁止跳过测试文件更新。
...
3. 前端AI必须检查每一个请求字段是否一致。
4. 前端AI必须检查每一个返回字段是否一致。
5. 前端AI必须检查每一个权限要求是否一致。
6. 前端AI必须检查侧边栏入口是否放在正确的工具组下。
7. 前端AI必须检查页面风格是否与当前系统一致。
8. 任一项不一致时，前端AI必须继续修改，禁止提交。
十六. 前端测试覆盖规则

1. 前端AI生成测试文件时，必须覆盖当前工具的全部前端功能。
2. 前端AI生成测试文件时，必须覆盖当前工具页面中全部可操作功能。
